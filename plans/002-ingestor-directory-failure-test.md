...
Conventions (from `docs/coding-standards.md` + the existing file): Pest `test( 'imperative behavior statement', function (): void { ... } )`; Arrange-Act-Assert; tab indent; `expect(...)->toBe(...)`; Brain Monkey `Functions\when(...)` for WP stubs. The test name states the expected behavior. Per the testing standard (`docs/testing.md`), this is pure-unit territory and the RED must be demonstrated.
