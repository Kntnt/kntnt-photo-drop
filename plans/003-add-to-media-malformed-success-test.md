...
Conventions (`agents.d/coding-standard/typescript.md` + `agents.d/coding-standard/wordpress-block.md`): block tests run on `@wordpress/scripts`' Jest — do not introduce Vitest. Tab indent, `it( 'behavior', async () => { ... } )`, the `result.ok` type-guard narrowing pattern shown above (the result is a discriminated union, so narrow with `if ( ! result.ok )` before reading `.status`).
