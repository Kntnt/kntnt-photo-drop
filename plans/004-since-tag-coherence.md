...
Conventions (`docs/coding-standards.md`): `@since` is required on every file/class/method/property/constant and "Include `@since` from the first release." Pre-1.0 policy (`AGENTS.md`): while major version is `0`, no back-compat concerns; pick the cleanest end-state.
