...
The eleven ADRs (`docs/adr/0001`–`0011`) own the decisions with real trade-offs: filesystem collections with no Media Library (0001), the immutable WebP output contract (0002), the on-disk layout and mtime-validated index (0003), the grouped CLI with consumer `import` (0004), the recursive-flatten gallery (0005), the server-enforced contract behind a nonce + `upload_files` REST upload (0006), the Interactivity-API lightbox (0007), hierarchy-preserving folder drop and immutable per-uploader folders (0008), the passive fullscreen slideshow with a pluggable trigger (0009), the token-wired Drop Zone upload controls (0010), and the slideshow's cycle-boundary resync to the gallery's current view by page refetch (0011). **Never contradict design.md or an ADR. Never redesign.** If a task seems to require contradicting a decision, stop and surface it — change is an ADR, not a silent edit.
