...
- **Contributor standards and process documentation only — not code.** Hardened the test and agent-workflow standards ahead of the redesign build-out. The **TDD rule** now requires the *red* step to be demonstrated — a test seen failing before the code that satisfies it, since a test never observed to fail is of unknown value — and to automate at the lowest layer that meaningfully constrains behaviour, reserving human checks for the irreducibly subjective (`coding-standards.md`, `definition-of-done.md`). A documented **autonomous multi-agent operating model** was added (`AGENTS.md`): agents never block on the maintainer, resolve ambiguity by recorded assumption, stop a single unit on a true ADR-level blocker rather than guess past it, and report upward in three buckets — *automatically tested* / *remaining for a human* / *assumptions & blockers* — which the outermost agent aggregates into one end-of-work report. `testing.md` gained per-issue test-layer guidance for the redesign issues (#42–#53), and the slideshow ADR's superseded caption mentions are annotated to match its amendment banner.
