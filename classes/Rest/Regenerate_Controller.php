...
class Regenerate_Controller {
	...
	private function required_capability(): string {
		...
		$filtered = apply_filters( 'kntnt_photo_drop_manage_capability', self::DEFAULT_CAPABILITY );
	 ...
	 *
	 * Prefers the canonical `X-WP-Nonce` header that `wp.apiFetch` sends, falling back
	 * to a `_wpnonce` parameter. The value is sanitised before it reaches
	 * `wp_verify_nonce()`.
	 *
	 * @since 0.11.0
	 *
	...
	private function read_nonce( \WP_REST_Request $request ): string {
		...
		// Take the header value first, then the parameter fallback; sanitise either way
		// so only a clean token string reaches the verifier.
	 ...
	 * access. The request body's own `slug`/`newSlug` fields are never consulted, so a
	 * regenerate can never rename a collection.
	 *
	 * @since 0.11.0
	 *
	 * @param \WP_REST_Request $request The incoming REST request.
