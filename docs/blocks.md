...
- Validation: slug lowercase, URL-safe, unique among existing directories (a typed collision errors; a blank field auto-suffixes); the path-components template is normalised with `%` reserved (a stray `%` after the four placeholders errors); a *present* width must be at least 320 px (a smaller value renders an effectively invisible gallery) and a *present* quality is 0–100, but Upload width, Full width, Full quality, and Thumbnail width may each be left blank (upload width blank = original dimensions; the re-derivable trio blank = unset/collapse-to-parent).
