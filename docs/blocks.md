...
- **Download** is the sole download trigger, always targeting the **main image**. The icon is an `<a download>` anchor, but with JavaScript a plain click is intercepted and the image saved **programmatically** — fetched into a Blob and handed to the browser through a temporary same-document object-URL anchor — so the save can never be hijacked into navigation or a new tab. Modified clicks (Cmd/Ctrl/Shift/Alt, non-primary button) are left to the browser, and the `<a download>` semantics are the no-JS fallback.
