...
Folder structure is preserved through a **"Select folder"** control (`webkitdirectory`): each file's `webkitRelativePath` is carried per file; the server recreates the sub-directories (path hard-sanitised and `realpath`-confined). Plain drag-and-drop onto the surface handles loose files, and a click anywhere on the surface opens a hidden loose-file picker. A *folder* dragged onto the drop zone is detected (`webkitGetAsEntry().isDirectory`, no recursion) and **warned** about, offering to continue flat; recursive drag traversal is out of scope.
