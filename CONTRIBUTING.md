...
- **Code** follows the per-module coding standard under [`agents.d/coding-standard/`](agents.d/coding-standard/). Note the four deliberate deviations from the WordPress Coding Standards – `[ ]` arrays, PSR-4 filenames, namespaces over global function prefixes and no required Yoda conditions – which are enforced in `phpcs.xml.dist` and must not be ‘corrected’ toward upstream WP-CS. Block JS/TS stays on the `@wordpress/scripts` happy path (its bundled ESLint, Prettier, Stylelint and Jest).
